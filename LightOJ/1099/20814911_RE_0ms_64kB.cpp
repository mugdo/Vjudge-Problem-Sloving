...
bool operator < (st x,st y)
{
    if(x.w!= y.w)
        ...
        x.seceond_wet>y.seceond_wet;

}
int diskta(int sorce,int n,vector<int>v[50010])
{
   for(int i=0;i<10;i++)
   {
       vis[i]=0;
   ...
   while(!pq.empty())
   {
       ...
       for(int i=0;i<v[ob.p].size();i++)
       {
          ...
          // cout<<"************************"<<endl;
           if(dis[v[ob.p][i]].first>(dis[ob.p].first+wet[ob.p][v[ob.p][i]]))
           {
              int oldVlue= dis[v[ob.p][i]].first;
              dis[v[ob.p][i]].first=(dis[ob.p].first+wet[ob.p][v[ob.p][i]]);
           ...
           else if(dis[v[ob.p][i]].second>(dis[ob.p].first+wet[ob.p][v[ob.p][i]])&&dis[v[ob.p][i]].first<(dis[ob.p].first+wet[ob.p][v[ob.p][i]]))
           {
                st obj2;
                obj2.p=v[ob.p][i];
                dis[v[ob.p][i]].second=(dis[ob.p].first+wet[ob.p][v[ob.p][i]]);
                obj2.seceond_wet=dis[v[ob.p][i]].second;
                obj2.w=dis[v[ob.p][i]].first;
                pq.push(obj2);
                //cout<<"222222dis["<<v[ob.p][i]<<"].second="<<dis[v[ob.p][i]].second<<endl;
            }
            else if(dis[v[ob.p][i]].second>(dis[ob.p].second+wet[ob.p][v[ob.p][i]])&&dis[v[ob.p][i]].first<(dis[ob.p].second+wet[ob.p][v[ob.p][i]]))
            {
                st obj2;
                obj2.p=v[ob.p][i];
                dis[v[ob.p][i]].second=(dis[ob.p].second+wet[ob.p][v[ob.p][i]]);
                obj2.seceond_wet=dis[v[ob.p][i]].second;
                obj2.w=dis[v[ob.p][i]].first;
                pq.push(obj2);
               // cout<<"222222dis["<<v[ob.p][i]<<"].second="<<dis[v[ob.p][i]].second<<endl;
...
int main()
{
    ...
    for(int T=0;T<t;T++)
    {
        ...
        cout<<"Case "<<T+1<<": "<<diskta(1,n,v)<<endl;
